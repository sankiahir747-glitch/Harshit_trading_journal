...
st.markdown("""
    ...
    /* Custom Table Styling for Heavyweights */
    .heavyweights-table {
        width: 100%;
        border-collapse: collapse;
        margin-top: 10px;
    }
    .heavyweights-table th {
        background-color: #F3F4F6;
        padding: 10px;
        text-align: left;
...
with col_status:
    ...
    if st.button("🔄 Refresh Live Market", use_container_width=True):
        st.rerun()

# FAST ROBUST REAL-TIME DERIVATIVE INDEX TICKER STREAMING
...
def fetch_realtime_pure_indices():
    ...
    # High-reliability fallback values in case yfinance latency occurs
    fallback_defaults = {
        "NIFTY 50": (24288.25, 45.20, 0.19),
        "BANKNIFTY": (57030.75, -160.10, -0.28),
        "SENSEX": (77843.10, 185.30, 0.24),
        "FINNIFTY": (26178.95, -108.20, -0.41),
        "MIDCAP NIFTY": (58910.40, 120.15, 0.20),
        "BSE BANKEX": (64662.70, -228.40, -0.35),
        "NIFTY IT": (31450.50, 398.20, 1.28),
        "INDIA VIX": (12.85, -0.35, -2.65)
    }
...
# ==========================================
# 4. HEAVYWEIGHTS WITH COLOR-CODED TABLE (IMAGE 2 FIX)
# ==========================================
with tab_components:
    ...
    # Custom HTML Table with Green and Red Price Change Highlights
    table_html = """
    <table class="heavyweights-table">
        <thead>
            <tr>
                <th>Stock Name</th>
                <th>Ticker</th>
    ...
    for name, sym in selected_dict.items():
        try:
            ...
            if len(hist) >= 5:
                ...
                color_class = "text-green" if pct >= 0 else "text-red"
                ...
                <tr>
                    ...
                    <td><code>{sym}</code></td>
                    <td>₹{cp:,.2f}</td>
                    <td class="{color_class}">{sign_str}{pct:.2f}%</td>
...
# ==========================================
# 5. SECTOR PERFORMANCE (ALL 17 SECTORS + % LABELS - IMAGE 3 FIX)
# ==========================================
with tab_sector:
    ...
    # Fallback simulated data array if yfinance API delays
    ...
    # Create Full-Width Plotly Bar Chart with Exact % Text Labels on Bars
    fig_sec = px.bar(
        sec_df, 
        y="Sector", 
        x="Change (%)", 
