...
# TODAY'S MIDNIGHT AUTO-RESET BANNER
...
# ==========================================
# 1. FAST TRADE ENTRY & MULTI-MARKET
# ==========================================
with tab_entry:
    ...
    with col_input1:
        ...
        symbol = st.text_input("Symbol / Instrument", placeholder="e.g. NIFTY50, BTCUSDT, EURUSD").upper()
    ...
    with col_input3:
        ...
        if strategy_option == "Other (Custom)":
            strategy = st.text_input("Custom Strategy Name", placeholder="e.g. VWAP Reversal")
        else:
...
with tab_calc:
    ...
    if sl_points > 0:
        ...
        st.info("Set valid Entry & SL prices to calculate position size.")
...
with tab_ai:
    ...
    if df.empty:
        st.info("Log trades first to allow AI analysis.")
    else:
        tot_tr = len(df)
        wins_df = df[df['Net_PnL'] > 0]
        loss_df = df[df['Net_PnL'] < 0]
        win_rate = (len(wins_df) / tot_tr) * 100
        net_pnl_tot = df['Net_PnL'].sum()
        
        top_strat = df.groupby('Strategy')['Net_PnL'].sum().idxmax()
        worst_mistake = df.groupby('Mistakes')['Net_PnL'].sum().idxmin()
        
        st.markdown(f"""
        <div class="ai-card">
            <h4>💡 AI Trading Performance Insight:</h4>
            <ul>
                <li><strong>Execution Rate:</strong> Out of <b>{tot_tr} trades</b>, your win rate is <b>{win_rate:.1f}%</b> with a total Net Realized P&L of <b>₹{net_pnl_tot:,.2f}</b>.</li>
                <li><strong>Best Performing Strategy:</strong> Your highest edge strategy is <b>'{top_strat}'</b>. Focus more on this setup.</li>
                <li><strong>Biggest P&L Leakage:</strong> Most of your losses are coming from the mistake: <b>'{worst_mistake}'</b>. Eliminating this can improve your bottom line directly.</li>
                <li><strong>Behavioral Recommendation:</strong> Check psychology logs whenever you feel <i>FOMO</i> or <i>Revenge</i>; execute strictly within your Position Sizing limits.</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)

# ==========================================
# 4. ADVANCED ANALYTICS (35+ REPORTS)
# ==========================================
with tab_analytics:
    ...
    if not df.empty:
        ...
        win_r = (len(w_df)/tot_t)*100
        tot_w_amt = w_df['Net_PnL'].sum()
        tot_l_amt = abs(l_df['Net_PnL'].sum())
        profit_factor = round(tot_w_amt/tot_l_amt, 2) if tot_l_amt > 0 else tot_w_amt
        avg_w = w_df['Net_PnL'].mean() if not w_df.empty else 0
        avg_l = abs(l_df['Net_PnL'].mean()) if not l_df.empty else 0
        ...
        fig_equity = px.line(df_sorted, x="Date", y="Cum_PnL", title="Equity Curve Growth", markers=True)
...
with tab_calendar:
    ...
    if not df.empty:
        ...
        m_net = f_df['Net_PnL'].sum() if not f_df.empty else 0.0
...
with tab_manage:
    ...
    if not df.empty:
        ...
        with c_e:
            ...
            st.download_button("📥 Export Journal CSV", data=csv, file_name=f"Harshit_Journal_{today_str}.csv", mime="text/csv", use_container_width=True)
