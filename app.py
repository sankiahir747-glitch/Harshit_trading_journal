...
# ULTRA-CLEAN LIGHT THEME WITH BULL/BEAR & CANDLESTICK WATERMARK
st.markdown("""
    <style>
    .stApp {
        ...
        background-image: 
            ...
            url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" opacity="0.025"><path d="M20 10v40m10-30v20m20-30v50m10-40v30m20-20v30" stroke="%23D97706" stroke-width="2" fill="none"/></svg>');
...
# Helper function for Financial Year calculation
...
# ==========================================
# MODULE 1: REAL-TIME MARKET TICKER BANNER
# ==========================================
@st.cache_data(ttl=60)
def fetch_live_index_ticker():
    tickers = {"NIFTY 50": "^NSEI", "BANKNIFTY": "^NSEBANK", "SENSEX": "^BSESN", "RELIANCE": "RELIANCE.NS"}
    data = {}
...
live_data = fetch_live_index_ticker()
if live_data:
    cols_t = st.columns(len(live_data))
    for idx, (k, (price, pct)) in enumerate(live_data.items()):
        sign = "+" if pct >= 0 else ""
        cols_t[idx].metric(k, f"₹{price:,.2f}", f"{sign}{pct:.2f}%")

st.markdown("""
    <div class="quote-banner">
        <span class="quote-text">📈 TRADER RULE: "Small profits are better than big losses. Protect your capital first, profits will follow." 🐂📊</span>
    </div>
""", unsafe_allow_html=True)
...
# NAVIGATION TABS
tab_entry, tab_calc, tab_strategy, tab_analytics, tab_sector, tab_news, tab_calendar, tab_manage = st.tabs([
    "⚡ Fast Trade Entry", 
...
# ==========================================
# 1. FAST TRADE ENTRY & REAL-TIME PRICE FETCH
# ==========================================
with tab_entry:
    ...
    with col_input1:
        ...
        if symbol_raw:
            try:
                ...
                fast_data = t_ticker.fast_info
                live_price_val = fast_data['lastPrice']
                st.info(f"📡 Real-Time Live Market Price ({sym_lookup}): **₹{live_price_val:,.2f}**")
            except Exception:
                pass
...
# ==========================================
# MODULE 5: SECTOR FLOW & FII/DII DATA
# ==========================================
with tab_sector:
    st.subheader("📊 Sectoral Performance & FII / DII Daily Activity Tracker")
    
    st.markdown("#### 🏦 Sectoral Indices Live Performance")
    sectors = {
        "Nifty Bank": "^NSEBANK",
        "Nifty IT": "^CNXIT",
        "Nifty Auto": "^CNXAUTO",
    ...
    for name, sym in sectors.items():
        try:
            ...
            if len(h) >= 1:
                cp = h['Close'].iloc[-1]
                prev = h['Close'].iloc[-2] if len(h) >= 2 else cp
    ...
    if sec_data:
        ...
        fig_sec = px.bar(sec_df, x="Sector", y="Change (%)", color="Change (%)", color_continuous_scale=["#EF4444", "#10B981"], title="Sectoral Heatmap")
        st.plotly_chart(fig_sec, use_container_width=True)

    st.divider()
    st.markdown("#### 🏛️ FII / DII Institutional Net Activity (Recorded Tracker)")
    fii_sample = pd.DataFrame([
    ...
    st.dataframe(fii_sample, use_container_width=True)

# ==========================================
# MODULE 6: LIVE INDIAN & GLOBAL NEWS
# ==========================================
with tab_news:
    st.subheader("📰 Live Indian Market & Global Financial News")
    try:
        feed = feedparser.parse("https://news.google.com/rss/search?q=Indian+Stock+Market+Nifty&hl=en-IN&gl=IN&ceid=IN:en")
        for entry in feed.entries[:8]:
            st.markdown(f"🔹 **[{entry.title}]({entry.link})**")
            st.caption(f"Published: {entry.published}")
            st.divider()
    except Exception:
        st.info("Live market news feed loading...")

# ==========================================
# MODULE 2: DYNAMIC MULTI-YEAR TRADER'S DIARY (2020-2030+)
# ==========================================
with tab_calendar:
    st.subheader("📅 Trader's Diary & Financial Year Report")
    
    # Generate dynamic multi-year options (e.g. FY 2020-21 to FY 2030-31)
    generated_fys = [f"FY {y}-{str(y+1)[-2:]}" for y in range(2020, 2031)]
    
    c_fy, c_mo = st.columns(2)
    with c_fy:
        selected_fy = st.selectbox("Select Financial Year (FY):", generated_fys, index=6) # Default FY 2026-27
    ...
    # Financial Year Month to Cal Year Mapping
