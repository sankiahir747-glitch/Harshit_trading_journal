...
try:
    ...
    st_autorefresh(interval=30000, key="live_market_refresher")
except Exception:
    pass
...
# ==========================================
# UPDATE 1 & 5: REAL-TIME NSE/BSE TICKER AUTO RUN
# ==========================================
...
live_ticks = fetch_realtime_indices()
if live_ticks:
    cols_t = st.columns(len(live_ticks))
    for idx, (k, (price, pct)) in enumerate(live_ticks.items()):
        sign = "+" if pct >= 0 else ""
...
with tab_analytics:
    ...
    if df.empty:
        st.warning("⚡ Currently 0 trades logged. Log trades to view equity growth & expectancy metrics.")
    else:
        tot_t = len(df)
...
# ==========================================
# MODULE 5: SECTOR FLOW & FII/DII TRACKER
# ==========================================
with tab_sector:
    st.subheader("📊 Sectoral Flow (Nifty 50) & Daily FII/DII Data")
    
    st.markdown("#### 🏦 Nifty 50 & Key Sectoral Indices Live Heatmap")
    sec_symbols = {
        "Nifty Bank": "^NSEBANK",
        "Nifty IT": "^CNXIT",
        "Nifty Auto": "^CNXAUTO",
    ...
    if sec_data:
        ...
        fig_sec = px.bar(sec_df, x="Sector", y="Change (%)", color="Change (%)", color_continuous_scale=["#EF4444", "#10B981"], title="Real-Time Sector Flow")
...
# ==========================================
# MODULE 6: LIVE NEWS, CRYPTO, FOREX & GLOBAL IMPACT
# ==========================================
with tab_news:
...
with tab_calendar:
    ...
    # Generated Multi-Year Financial Years List (2020 to 2030)
