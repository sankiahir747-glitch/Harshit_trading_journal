...
try:
    ...
    st_autorefresh(interval=30000, key="global_terminal_30s_autorefresh")
except Exception:
    pass

# ULTRA-CLEAN LIGHT THEME WITH RED/GREEN HTML TABLE STYLING
st.markdown("""
    <style>
    .stApp {
    ...
    /* CUSTOM HTML TABLE FOR GLOBAL MARKETS & FUTURES */
    .market-styled-table {
        width: 100%;
    ...
    .market-styled-table td {
        padding: 10px 12px;
        border-bottom: 1px solid #E5E7EB;
        font-size: 14px;
        font-weight: 600;
    }
    .green-val { color: #10B981 !important; font-weight: 700; }
    .red-val { color: #EF4444 !important; font-weight: 700; }
    .badge-bullish { background-color: #DCFCE7; color: #15803D; padding: 4px 8px; border-radius: 6px; font-weight: bold; }
    .badge-bearish { background-color: #FEE2E2; color: #B91C1C; padding: 4px 8px; border-radius: 6px; font-weight: bold; }
    .badge-neutral { background-color: #F3F4F6; color: #4B5563; padding: 4px 8px; border-radius: 6px; font-weight: bold; }
...
# ==========================================
# 2. CRYPTO & FOREX TICKERS (30s AUTO + REFRESH BTN)
# ==========================================
...
# ==========================================
# HELPER FUNCTION FOR HIGH CONTRAST HTML MARKET TABLES
# ==========================================
def render_market_html_table(symbols_dict):
    html_code = """
    <table class="market-styled-table">
        <thead>
            <tr>
                <th>Name</th>
                <th>LTP</th>
        ...
        try:
            ...
            if len(hist) >= 2:
                ...
                if pct > 0.8 and cp > sma_20:
                    rating_badge = '<span class="badge-bullish">Very Bullish 📈</span>'
                elif pct > 0:
                    rating_badge = '<span class="badge-bullish">Bullish ↗️</span>'
                elif pct < -0.8 and cp < sma_20:
                    rating_badge = '<span class="badge-bearish">Very Bearish 📉</span>'
                elif pct < 0:
                    rating_badge = '<span class="badge-bearish">Bearish ↘️</span>'
                else:
                    rating_badge = '<span class="badge-neutral">Neutral ➖</span>'

                color_cls = "green-val" if pct >= 0 else "red-val"
                sign = "+" if pct >= 0 else ""
                ...
                <tr>
                    <td><b>{name}</b></td>
                    <td>{cp:,.2f}</td>
                    <td class="{color_cls}">{sign}{chg:,.2f}</td>
                    <td class="{color_cls}">{sign}{pct:.2f}%</td>
                    <td>{high_val:,.2f}</td>
                    <td>{low_val:,.2f}</td>
                    <td>{open_val:,.2f}</td>
                    <td>{prev:,.2f}</td>
...
# ==========================================
# 3. GLOBAL MARKETS (HIGH CONTRAST COLOR + FLAGS)
# ==========================================
with tab_global_mkt:
    ...
    st.markdown(render_market_html_table(us_dict), unsafe_allow_html=True)

    st.markdown("#### 🇪🇺 EUROPEAN MARKETS")
    eu_dict = {"🇬🇧 FTSE 100 (UK)": "^FTSE", "🇫🇷 CAC 40 (France)": "^FCHI", "🇩🇪 DAX (Germany)": "^GDAXI"}
    st.markdown(render_market_html_table(eu_dict), unsafe_allow_html=True)
    ...
    st.markdown(render_market_html_table(asia_dict), unsafe_allow_html=True)
...
with tab_futures_etfs:
    ...
    st.markdown(render_market_html_table(futures_dict), unsafe_allow_html=True)
    ...
    st.markdown(render_market_html_table(etf_dict), unsafe_allow_html=True)
