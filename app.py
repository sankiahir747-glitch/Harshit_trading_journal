...
# Page Configuration
st.set_page_config(
    page_title="Harshit Terminal | Institutional OS",
    page_icon="👑",
    layout="wide"
)

# Custom Institutional Bloomberg/Cyber-Terminal Styling
st.markdown("""
    <style>
    .stApp {
    ...
    div[data-testid="stMetricValue"] {
        ...
        text-shadow: 0px 0px 10px rgba(245, 158, 11, 0.2);
    }
...
if not df.empty:
    ...
    if today_net < -MAX_DAILY_LOSS:
        st.error(f"🛑 RISK WARNING: Max Daily Loss Limit Exceeded (₹{today_net:,.2f}). Stop trading for today!")
...
# ==========================================
# 3. STRATEGY EDGE & ACCURACY % (NEW DEDICATED MODULE)
# ==========================================
with tab_strategy:
    st.subheader("🎯 Strategy Edge & Accuracy (%) Breakdown")
    if df.empty:
        st.info("Log trades to view strategy accuracy stats.")
    else:
        strat_stats = []
        ...
        st.divider()
        st.markdown("#### 📊 Strategy Accuracy Comparison Chart")
        fig_strat_acc = px.bar(
            ...
            title="Strategy Win-Rate Accuracy (%) Comparison",
            template="plotly_dark"
        )
        st.plotly_chart(fig_strat_acc, use_container_width=True)

# ==========================================
# 4. ADVANCED ANALYTICS (35+ INSTITUTIONAL SUITE)
# ==========================================
with tab_analytics:
    ...
    if df.empty:
        ...
        st.divider()
        st.markdown("#### 2️⃣ Equity Growth Curve & P&L Analysis")
        df_sorted = df.sort_values("Date").reset_index(drop=True)
        df_sorted['Cum_PnL'] = df_sorted['Net_PnL'].cumsum()
        
        fig_equity = px.line(df_sorted, x="Date", y="Cum_PnL", title="Equity Growth Curve", markers=True, template="plotly_dark")
        ...
        with col_an1:
            ...
            fig_type = px.bar(type_grp, x='Type', y='sum', color='Type', title="P&L by Trade Type", template="plotly_dark")
            st.plotly_chart(fig_type, use_container_width=True)
            
        with col_an2:
            st.markdown("#### 4️⃣ Performance by Emotion")
            emo_grp = df.groupby('Emotion')['Net_PnL'].sum().reset_index()
            fig_emo = px.bar(emo_grp, x='Emotion', y='Net_PnL', color='Net_PnL', title="Net P&L by Emotional State", template="plotly_dark")
        ...
        try:
            ...
            <div class="terminal-card">
                ...
                <ul>
                    ...
                    <li><strong>Behavioral Guidance:</strong> Always calculate position size before trade entry to keep emotions disciplined.</li>
                </ul>
            </div>
            """, unsafe_allow_html=True)
        except Exception:
...
st.markdown("""
    <div class="profile-card">
        ...
        <p><strong>Instagram:</strong> <a href="https://instagram.com/harshityadu1c_" target="_blank" style="color: #F59E0B;">@harshityadu1c_</a> | 
           <strong>Twitter (X):</strong> <a href="https://twitter.com/harshityadu1c_" target="_blank" style="color: #F59E0B;">@harshityadu1c_</a> | 
