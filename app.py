...
# AUTOMATIC SMOOTH AUTO-REFRESH (30 Seconds - Zero Glitch)
try:
...
# ULTRA-CLEAN LIGHT THEME WITH HIGHLIGHTED TAB STYLING
st.markdown("""
    <style>
    .stApp {
...
# TABS NAVIGATION (MEDIUM & HIGHLIGHTED)
...
# ==========================================
# 3. GLOBAL MARKETS (FLAG + TECHNICAL RATING + REFRESH BTN)
# ==========================================
with tab_global_mkt:
    ...
    def get_global_market_rated_table(symbols_dict):
        table_rows = []
        for name, sym in symbols_dict.items():
            try:
                ...
                if len(hist) >= 2:
                    ...
                    if pct > 1.0 and cp > sma_20:
                        rating = "Very Bullish 📈"
                    elif pct > 0:
                        rating = "Bullish ↗️"
                    elif pct < -1.0 and cp < sma_20:
                        rating = "Very Bearish 📉"
                    elif pct < 0:
                        rating = "Bearish ↘️"
                    else:
    ...
    us_df = get_global_market_rated_table(us_dict)
    if not us_df.empty:
    ...
    eu_df = get_global_market_rated_table(eu_dict)
    if not eu_df.empty:
        st.dataframe(eu_df, use_container_width=True)

    st.markdown("#### 🌏 ASIAN MARKETS")
    asia_dict = {"🇯🇵 Nikkei 225 (Japan)": "^N225", "🇭🇰 Hang Seng (Hong Kong)": "^HSI", "🇮🇳 GIFT Nifty": "GIFTNIFTY.NS"}
    asia_df = get_global_market_rated_table(asia_dict)
    if not asia_df.empty:
        st.dataframe(asia_df, use_container_width=True)

# ==========================================
# UPDATE 3: FUTURES & ETFS SECTION
# ==========================================
with tab_futures_etfs:
    col_fe_title, col_fe_btn = st.columns([4, 1])
    with col_fe_title:
        st.subheader("📈 Global Futures & Key Traders' ETFs")
    with col_fe_btn:
    ...
    futures_dict = {
        ...
        "Copper Futures": "HG=F",
        "US 10Y Treasury Bond Yield": "^TNX"
    }
    
    fut_df = get_global_market_rated_table(futures_dict)
    if not fut_df.empty:
    ...
    etf_df = get_global_market_rated_table(etf_dict)
    if not etf_df.empty:
