...
# AUTOMATIC FAST AUTO-REFRESH (Every 2 Seconds for Live Prices & Blinking Dot)
try:
    from streamlit_autorefresh import st_autorefresh
    st_autorefresh(interval=2000, key="live_market_tick_stream")
except Exception:
    pass
...
st.markdown("""
    ...
    /* Blinking Live Indicator Styling */
...
with col_status:
    ...
    # Check Market Hours (Mon-Fri 9:15 AM to 3:30 PM IST)
...
# REAL-TIME DERIVATIVE INDEX TICKER STREAMING
@st.cache_data(ttl=2)  # 2 second live cache refresh
...
if live_ticks:
    ...
    for idx in range(min(4, len(keys))):
        k = keys[idx]
        price, chg, pct = live_ticks[k]
        sign = "+" if pct >= 0 else ""
