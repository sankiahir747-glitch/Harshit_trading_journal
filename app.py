...
# TABS NAVIGATION
tab_entry, tab_crypto_forex, tab_global_mkt, tab_calc, tab_strategy, tab_components, tab_sector, tab_news, tab_calendar, tab_manage, tab_profile = st.tabs([
    "⚡ Fast Trade Entry",
    "🪙 Crypto & 💵 Forex Tickers",
...
# ==========================================
# UPDATE 1 & 2: CRYPTO & FOREX TICKERS
# ==========================================
with tab_crypto_forex:
    st.subheader("🪙 Crypto & 💵 Forex Live Market Tickers")
...
# ==========================================
# UPDATE 3: GLOBAL MARKETS OVERVIEW (EXACT LAYOUT BASED ON SHARED IMAGE)
# ==========================================
with tab_global_mkt:
    st.subheader("🌍 Global Market Indices & Macro Overview")
    
    def get_global_market_table(symbols_dict):
        table_rows = []
    ...
    us_df = get_global_market_table(us_dict)
    if not us_df.empty:
    ...
    eu_df = get_global_market_table(eu_dict)
    if not eu_df.empty:
    ...
    asia_df = get_global_market_table(asia_dict)
    if not asia_df.empty:
        st.dataframe(asia_df, use_container_width=True)

    st.markdown("#### 🛢️ COMMODITIES & BONDS")
    comm_dict = {"Gold": "GC=F", "Silver": "SI=F", "Brent Crude Oil": "BZ=F", "US 10Y Bond Yield": "^TNX"}
    comm_df = get_global_market_table(comm_dict)
