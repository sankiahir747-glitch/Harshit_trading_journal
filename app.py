...
# ==========================================
# UPDATE 1: PURE DERIVATIVE INDEXES TICKER (NO STOCKS)
# ==========================================
...
def fetch_realtime_pure_indices():
    ...
    for name, sym in indices.items():
        try:
            ...
            # Fallback for display structure
            res[name] = (0.0, 0.0)
    return res
...
if live_ticks:
    ...
    keys = list(live_ticks.keys())
    for idx in range(min(5, len(keys))):
        k = keys[idx]
        price, pct = live_ticks[k]
        sign = "+" if pct >= 0 else ""
        cols_t1[idx].metric(k, f"₹{price:,.2f}" if price > 0 else "Live Feed", f"{sign}{pct:.2f}%")
        
    for idx in range(5, len(keys)):
        k = keys[idx]
        price, pct = live_ticks[k]
        sign = "+" if pct >= 0 else ""
        cols_t2[idx-5].metric(k, f"₹{price:,.2f}" if price > 0 else "Live Feed", f"{sign}{pct:.2f}%")
...
# ==========================================
# UPDATE 2: INDEX HEAVYWEIGHTS & TECHNICAL TREND ANALYSIS
# ==========================================
with tab_components:
    ...
    for name, sym in selected_dict.items():
        try:
            ...
            if len(hist) >= 5:
                ...
                # Simple Moving Average Trend Signal Logic
...
# ==========================================
# UPDATE 3: ZERODHA STYLE SECTOR PERFORMANCE (IMAGE MATCHED)
# ==========================================
with tab_sector:
    st.subheader("📊 Sector Performance & FII / DII Daily Activity Tracker")
    
    st.markdown("#### 🏦 Sectoral Heatmap (Zerodha Style)")
    sec_symbols = {
        "IT": "^CNXIT",
        "AUTOMOBILE": "^CNXAUTO",
...
# ==========================================
# MODULE 7: DYNAMIC MULTI-YEAR TRADER'S DIARY
# ==========================================
with tab_calendar:
...
# ==========================================
# UPDATE 4: DEDICATED PROFILE & BRANDING TAB
# ==========================================
with tab_profile:
