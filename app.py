        ...
        try:
            ...
            for col in cols:
                if col not in df.columns:
                    ...
                    elif col in ["Brokerage", "Other_Charges", "Total_Charges", "Gross_PnL", "Net_PnL", "PnL_Pct", "Risk_Reward", "Hold_Time_Mins"]:
                        df[col] = 0.0
                    elif col == "Market":
...
# ==========================================
# 1. FAST TRADE ENTRY (MULTIMARKET ARCHITECTURE)
# ==========================================
with tab_entry:
    st.subheader("📝 Universal Multi-Market Trade Logger")
    
    # 1. UNIVERSAL FIELDS
    st.markdown("#### 1️⃣ Universal Fields")
    ...
    with col_u2:
        ...
        symbol_raw = st.text_input("Asset / Ticker Name", placeholder="e.g. NIFTY, RELIANCE, EURUSD, BTCUSDT").upper()
    with col_u3:
    ...
    # 2. MARKET-SPECIFIC FIELDS
    st.markdown("#### 2️⃣ Market-Specific Dynamic Fields")
    ...
    if market_segment == "Nifty/BankNifty (Options)":
        ...
        with col_m1:
            leverage = st.selectbox("Leverage Used", ["1x", "5x", "10x", "20x", "50x", "100x"])
        with col_m2:
            margin_allocated = st.number_input("Margin Allocated ($/₹)", min_value=0.0, step=100.0)
            
    elif market_segment == "Equity/Stocks":
        with col_m1:
            trade_exec_type = st.selectbox("Trade Type", ["Intraday (MIS)", "Delivery (CNC)"])

    # 3. PSYCHOLOGY & JOURNALING FIELDS
    st.markdown("#### 3️⃣ Psychology & Journaling Fields")
    col_p1, col_p2, col_p3 = st.columns(3)
    
    with col_p1:
        strategy_option = st.selectbox("Setup / Strategy Used", ["Breakout", "Support/Resistance", "Trendline", "EMA Crossover", "Indicator-based", "Other (Custom)"])
    ...
    with col_p2:
        ...
        tags = st.text_input("Custom Tags", placeholder="Intraday, Scalp, Volatility")
    ...
    with col_p3:
        ...
        if mistake_option == "Other (Custom)":
            mistake = st.text_input("Custom Mistake Details")
        else:
            mistake = mistake_option

    col_notes1, col_notes2 = st.columns([2, 1])
    with col_notes1:
        notes = st.text_area("Detailed Trade Rationale & Notes")
    with col_notes2:
        ...
        chart_file = st.file_uploader("📸 Upload Chart Screenshot (TradingView)", type=["png", "jpg", "jpeg"])

    # 4. AUTO-CALCULATED BACKEND EXECUTION
    if st.button("🚀 Commit Trade To Terminal", use_container_width=True):
        if symbol_raw and entry_p > 0 and exit_p > 0 and sl > 0:
            ...
            if hold_time_mins < 0:
                hold_time_mins += 1440  # Overnight adjustment
            ...
            new_trade = pd.DataFrame([{
                ...
                "Trade_Execution_Type": trade_exec_type,
                "Brokerage": brokerage,
            ...
            save_data(df)
            st.success(f"✅ Trade Logged! Net P&L: ₹{net_pnl:.2f} ({pnl_pct:.2f}%) | R:R = 1:{rr_ratio} | Hold Duration: {hold_time_mins} Mins")
            st.rerun()
        else:
...
# ==========================================
# BULLETPROOF SAFE DATAFRAME BUILDER
# ==========================================
...
with tab_global_mkt:
    ...
    df_us = fetch_safe_market_dataframe(us_dict)
    if not df_us.empty:
    ...
    df_eu = fetch_safe_market_dataframe(eu_dict)
    if not df_eu.empty:
    ...
    df_asia = fetch_safe_market_dataframe(asia_dict)
    if not df_asia.empty:
        st.dataframe(df_asia, use_container_width=True)
    ...
    df_comm = fetch_safe_market_dataframe(comm_dict)
...
with tab_futures_etfs:
    ...
    df_fut = fetch_safe_market_dataframe(futures_dict)
    if not df_fut.empty:
